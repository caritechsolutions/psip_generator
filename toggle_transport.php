...
$action = $_GET['action'];
$statusFile = "/var/www/html/transports/{$transport}/status.ini";
...
$version = handleVersionFile($transport);
 echo "Loaded version: $version\n";
    ...
    $command = "/usr/bin/sudo /usr/bin/nohup /usr/bin/php /var/www/html/epg_gen.php $transport $version >/dev/null 2>&1 & echo $!";
    exec($command);

 sleep(1);

    $command = "/usr/bin/sudo /usr/bin/nohup /usr/bin/php /var/www/html/runtsduck.php $transport $version >/dev/null 2>&1 & echo $!";
